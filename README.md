...
My general philosophy is that this should only contain small 'core' components with minimum interdependencies; larger components should be their own libraries. It should not contain platform-specific helpers.
...
- `FELLY_NO_UNIQUE_ADDRESS`: maps to either `[[no_unique_address]]` or `[[msvc::no_unique_address]]`; MSVC recognizes `[[no_unique_address]]` but ignores it, to avoid potential ABI issues
- `FELLY_CPLUSPLUS`: maps to either `__cplusplus` or `_MSVC_CLANG`; checks for and deals with CMake's handling of `CMAKE_CXX_STANDARD` with `clang-cl` (which passes `-clang:-std=c++23`)
